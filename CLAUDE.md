...
- **Section settings for layout**: `reverseOnMobile` (grid only, `cb-section--reverse-mobile`: stacked columns read last to first at ≤540px). A block's styling has `textAlign` (class `cb-block--text-*`), `maxWidth` and `alignSelf`. A background colour adds `cb-section--bg-dark|light` / `cb-block--bg-dark|light`, from `ContentBlocks\Palette\ColorTone` (also Twig `cb_color_tone()` / `cb_color_is_dark()`), the rule the library thumbnails use

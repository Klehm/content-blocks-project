...
│   ├── content-blocks-kit/      # ~18 blocs prêts à l'emploi, autonomes (kit.css)
...
The section sidebar's **Columns** group adds, names and removes columns (`ColumnsController`: `POST /section/{id}/columns`, `POST /column/{id}/delete`, `POST /column/{id}/settings`; CSRF + canEdit, journalled). Adding or removing re-spans the live columns to equal (undo restores them); the last live column cannot go; at most `MAX_COLUMNS` = 20. The `display` section setting (`SectionDisplay`: `grid` | `tabs` | `accordion`, also a layout option) shows the columns as **CSS-only tabs**: one radio per column, then `.cb-tabs__nav` labels, then `.cb-row`, as siblings so `:checked ~` reaches both (rules written out to 20 in `layout.css`), or as a **CSS-only accordion**: a checkbox and a `.cb-accordion__header` label before each column *inside* `.cb-row` (`:checked + label + .cb-col`, no per-index rule, panels independent; the row turns `display: block`). Anything walking columns must select `[data-cb-column-id]`, never the row's children by position. In the builder the overlay keeps the open tab/panels across reloads (`sessionStorage`, a list of column ids), opens a block's tab or panel when it is focused, and `patchSection` swaps the radios and nav (or re-inserts each toggle + header before its column) in place. Rationale: [docs/internals/rendering.md#columns-and-tabs](docs/internals/rendering.md#columns-and-tabs).

...
since 1.0.0-RC17. What the [backward compatibility](https://klehm.github.io/content-blocks-project/guide/backward-compatibility) page lists for
`klehm/content-blocks-kit` is stable until 2.0.

### Changed

- **`klehm/content-blocks` is required at `^1.0`**, the stable line.
...
Version bump only — no functional change in `klehm/content-blocks-kit`. The tag is cut
across the monorepo so the three packages stay installable as one set; this
candidate's change is in `klehm/content-blocks`: section display per viewport,
a slider display, and an order per viewport. No migration, nothing to wire.
...
this candidate's changes are in `klehm/content-blocks` (a "View page" link in
the builder, empty Twig blocks for host additions, `section.initial_settings`)
and `klehm/content-blocks-i18n` (per-language page links and a back-URL seam in
the workbench). No migration, no new Stimulus controller.
...
Version bump only — no functional change in `klehm/content-blocks-kit`. The tag
is cut across the monorepo so the three packages stay installable as one set;
this candidate's changes are in `klehm/content-blocks` (undo/redo, the
navigator, bundle-owned rows through export/import) and
`klehm/content-blocks-i18n` (translations through export/import). **The core
needs a migration** (`cb_action_log`): see its CHANGELOG and the
[upgrade guide](https://klehm.github.io/content-blocks-project/guide/upgrade).

...
final class AssetPolicy
{
     ...
     *
     * @throws ImportRefusedException when the file is refused
     */
    public function check(string $label, string $binary, string $claimedExtension = ''): string
    {
        if (\strlen($binary) > $this->uploadMaxSize) {
            throw new ImportRefusedException(sprintf('Asset %s is too large (max %d MB).', $label, intdiv($this->uploadMaxSize, 1024 * 1024), ));
        }

        $mime = (new \finfo(\FILEINFO_MIME_TYPE))->buffer($binary);
        if (!\is_string($mime) || !\in_array($mime, $this->uploadAllowedMimeTypes, true)) {
            throw new ImportRefusedException(sprintf('Asset %s: file type "%s" is not allowed.', $label, \is_string($mime) ? $mime : 'unknown', ));
        }
        ...
        if ($known === []) {
            throw new ImportRefusedException(sprintf('Asset %s: no extension for "%s".', $label, $mime));
        }

...
- **`cb-condition` Stimulus controller.** Generic conditional-field visibility: tag any row with `data-cb-condition="field:value1|value2"` inside a `data-controller="cb-condition"` scope (checkboxes map to `true`/`false`; `field` alone means "non-empty"; instances nest, nearest scope wins). Drives the styling switch and the palette's custom picker; reusable in host block forms.

...
This repo is a **monorepo**. Packages are published separately to Packagist via [splitsh/lite](https://github.com/splitsh/lite); the read-only mirrors (`klehm/content-blocks`, `klehm/content-blocks-kit`, `klehm/content-blocks-i18n`) are generated by CI. Contributors clone the monorepo and get everything — packages, three sandbox apps (Symfony with AssetMapper, Symfony 6.4 with Webpack Encore, Sylius), and the JS/PHP test suites.

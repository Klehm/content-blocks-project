...
| ybc — RC2 | **Un bloc sous-classé ne reçoit aucune de sa configuration.** `ContentBlocksKitBundle::loadExtension()` boucle sur `resolveBlocks()`, qui ne rend que des **classes du kit** : `options`, `choices` et `defaults` ne sont câblés que sur les services que le bundle enregistre lui-même. Or dès qu'un hôte sous-classe un bloc, il pose `enabled: false` — le type disparaît donc de `resolveBlocks()` et **toute sa configuration est silencieusement jetée**, la sous-classe étant autowirée par Symfony avec les valeurs par défaut du constructeur. Rien ne le signale : le YAML est valide, il ne s'applique simplement à rien. Constaté deux fois sur ybc — `rich_text.options` (l'éditeur repartait sur le CDN malgré `cdn: false`) puis `title.choices.size` (la variante `h1_variant` n'apparaissait pas). | Côté hôte : redéfinir `defaultOptions()` et `choiceFields()` dans la sous-classe, ce qui marche mais ramène en PHP ce que la RC2 venait justement d'ouvrir à la config. | **Oui.** Les `choices` en map ne servent que là où aucune sous-classe n'est nécessaire — c'est-à-dire là où le besoin est le plus faible. Dès qu'il manque un *champ*, ce que `choices` ne peut pas ajouter, il faut sous-classer et l'on reperd la config. Deux pistes : injecter `blocks.<type>` dans toute classe étendant `AbstractKitBlock` quel que soit son enregistrement, ou permettre à une sous-classe de déclarer le type dont elle hérite la configuration. |
| ybc — RC2 | **`TableBlock::defaults()` produit un alignement que le bloc lui-même refuse.** Il retourne `['label' => 'Name', 'align' => 'left']` et `['label' => 'Value', 'align' => 'right']`, valeurs d'avant la RC1 — alors que `TableColumnType` ne valide que `start`, `center` et `end`, et que le template ne mappe que ces trois-là, retombant sur « gauche » pour tout le reste. Un bloc `table` créé aujourd'hui démarre donc avec deux colonnes hors contrat, dont la seconde rend à gauche alors qu'elle s'annonce à droite. Le renommage `left`/`right` → `start`/`end` du §Changed de la RC1 a couvert le form type et la vue, pas les valeurs par défaut. | Sans effet sur le contenu déjà en base, qui passe par la migration. | **Oui.** Aligner `defaults()` sur `start`/`end`. Un test qui valide les `getDefaultData()` de chaque bloc contre son propre formulaire attraperait cette famille d'écarts d'un coup. |
